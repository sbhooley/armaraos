//! SSE stream auth: `/api/events/stream` and `/api/logs/stream` skip credentials only on loopback.
//! Remote clients must use `Authorization: Bearer` or `?token=` when `api_key` is set.
