...
function agentsPage() {
  return Object.assign(armaraosFleetVitalsCore(), {
    ...
    async init() {
      ...
      if (store.pendingOpenAgentDetail) {
        var pa = store.pendingOpenAgentDetail;
        store.pendingOpenAgentDetail = null;
        this.$nextTick(function() { self.showDetail(pa); });
      }
      if (store.pendingOpenSpawnWizard) {
        store.pendingOpenSpawnWizard = false;
        this.$nextTick(function() { self.openSpawnWizard(); });
      }
      this._onOpenSpawnWizard = function() { self.openSpawnWizard(); };
      try { document.addEventListener('armaraos-open-spawn-wizard', this._onOpenSpawnWizard); } catch (eW) { /* ignore */ }
    ...
    onAgentsPageLeave() {
      this.fleetTeardown();
      this.activeChatAgent = null;
      if (this._onOpenSpawnWizard) {
        try { document.removeEventListener('armaraos-open-spawn-wizard', this._onOpenSpawnWizard); } catch (eD) { /* ignore */ }
        this._onOpenSpawnWizard = null;
      }
      try {
        var st = Alpine.store('app');
        st.pendingAgent = null;
    ...
    /** Append mcp_* to a non-empty allowlist so saved manifests match kernel defaults (kernel merges mcp_* at runtime anyway). */
    ensureMcpGlobAllowlist() {
      var allow = this.toolFilters.tool_allowlist || [];
      if (allow.length === 0) return;
      var glob = 'mcp_*';
      if (allow.indexOf(glob) === -1 && allow.every(function(t) { return t !== '*'; })) {
        allow.push(glob);
      }
      this.toolFilters.tool_allowlist = allow;
      this.saveToolFilters();
