...
pub fn structural_prologue_only(proposed: &str) -> Result<(), String> {
    ...
    if t.len() > MAX_PROPOSED_AINL_BYTES {
        return Err(format!("proposed_ainl_text exceeds {MAX_PROPOSED_AINL_BYTES} bytes"));
    }
    let first = t
        .lines()
        .map(|l| l.trim())
    ...
    match first {
        ...
        _ => Err(
            "structural: first non-empty line must be `graph` (AINL file header)".to_string(),
        ),
    }
}
...
pub fn strict_line_shape(proposed: &str) -> Result<(), String> {
    ...
    let has_transform = t
        ...
        .any(|l| {
            let s = l.trim_start();
            s.starts_with("t1")
                || s.len() > 1 && s.starts_with('t') && s[1..].chars().next().is_some_and(|c| c.is_ascii_digit())
        });
    ...
    if has_transform || has_sec {
        ...
        Err(
            "strict: add at least one `tN` transform line or a `## ` top-level section".to_string(),
        )
    }
}
...
fn validate_external_process(proposed_ainl_text: &str) -> Result<(), String> {
    ...
    if !tpl.contains("%p") {
        return Err(format!("{ENV_EXT_VALIDATE} must include a single %p placeholder (temp .ainl path)"));
    }
