...
- **MCP resource bodies:** `mcp_ainl_ainl_capabilities` includes an `mcp_resources` list. Prefer **`ainl://strict-authoring-cheatsheet`**, **`ainl://strict-valid-examples`**, and **`ainl://adapter-contracts`** for strict syntax and contracts; use **`ainl://authoring-cheatsheet`** and **`ainl://integrations-*`** for HTTP `R`-line and integration notes. Read via **`mcp_resource_read`** with `mcp_server`=`ainl` (or `server`) and `uri`.
