...
use ainl_context_compiler::{ContextCompiler, Role as CcRole, Segment};
use dashmap::DashMap;
use openfang_types::message::{Message, Role as MsgRole};
...
///
/// Classification rules (M1):
/// - The system prompt → [`SegmentKind::SystemPrompt`].
/// - Each historical message becomes either a [`SegmentKind::RecentTurn`] (if among the last
///   `RECENT_TURN_THRESHOLD` non-system messages) or [`SegmentKind::OlderTurn`] otherwise.
/// - The trailing user message (the current turn) is replaced with an explicit
///   [`SegmentKind::UserPrompt`] using `latest_user_query` so the compiler treats it as
///   "always-keep" and scores other segments against it.
/// - Tool definitions and tool results are not yet split out (M2 work) — for M1 they are
///   counted as part of their containing message.
fn build_segments(
    system_prompt: &str,
    messages: &[Message],
    ...
    // The last message is almost always the current user turn; drop it so we can re-add it
    // as the canonical UserPrompt segment with the un-compressed query text.
    ...
    for (idx, msg) in history.iter().enumerate() {
        // age_index = 0 for the most recent message, increasing as we go back in time.
        let age_index = (history.len() - 1 - idx) as u32;
        let text = msg.content.text_content();
        if text.is_empty() {
            continue;
        }
        let role = map_role(msg.role);
        let seg = if idx >= recent_start {
            Segment::recent_turn(role, text, age_index)
        } else {
            Segment::older_turn(role, text, age_index)
        };
        segments.push(seg);
    }

    if !latest_user_query.is_empty() {
        segments.push(Segment::user_prompt(latest_user_query));
    } else if trailing_is_user {
        // No explicit query supplied — fall back to the trailing message content.
        if let Some(last) = messages.last() {
            let text = last.content.text_content();
            if !text.is_empty() {
                segments.push(Segment::user_prompt(text));
            }
        }
    }
...
/// Run the Tier 0 [`ContextCompiler`] over the assembled prompt and record telemetry for
/// the kernel to consume. **Measurement-only** in M1 — never mutates `messages` or the
/// system prompt; the LLM still receives the host-assembled prompt unchanged.
///
/// Failure modes are logged at `debug!` and treated as no-ops: when the compiler crashes or
/// produces zero-token output, the kernel falls back to its prior user-message-only
/// estimation path. This keeps M1 strictly additive — no production turn can fail because
/// of compose-side instrumentation.
pub fn measure_and_record(
    ...
    let segments = build_segments(system_prompt, messages, latest_user_query);
