/**
 * @armaraos/sdk — Official JavaScript client for the ArmaraOS REST API.
 *
 * Usage:
 *   const { ArmaraOS } = require("@armaraos/sdk");
 *   const client = new ArmaraOS("http://localhost:3000");
 *
...
class ArmaraOSError extends Error {
  constructor(message, status, body) {
    super(message);
    this.name = "ArmaraOSError";
    this.status = status;
    this.body = body;
  }
}
...
class ArmaraOS {
  /**
   * @param {string} baseUrl - ArmaraOS server URL (e.g. "http://localhost:3000")
   * @param {object} [opts]
  ...
  async _request(method, path, body) {
    ...
    if (!res.ok) {
      ...
      throw new ArmaraOSError("HTTP " + res.status + ": " + text, res.status, text);
    }
...
class AgentResource {
  ...
  async upload(id, file, filename) {
    ...
    if (!res.ok) throw new ArmaraOSError("Upload failed: " + res.status, res.status);
    return res.json();
  }
...
module.exports = { ArmaraOS: ArmaraOS, ArmaraOSError: ArmaraOSError };
