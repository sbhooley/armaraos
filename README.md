...
| **Self-learning** | [Self-learning map](docs/SELF_LEARNING_INTEGRATION_MAP.md): trajectory capture, **failure** recall blocks, **improvement proposals** (ledger + validate → adopt; **on by default**, `AINL_IMPROVEMENT_PROPOSALS_ENABLED=0`\|`false`\|`no`\|`off` to disable), **pattern promotion** for procedural memory, and dashboard/SSE integration — Hermes-style closed-loop learning without a separate silo. |
