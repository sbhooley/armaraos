...
**Per-agent graph memory (SQLite):** **`~/.armaraos/agents/<id>/ainl_memory.db`** (**`ainl-memory`** via **`GraphMemoryWriter`**, written from **`openfang-runtime`** on delegation and used for **persona** → system prompt). Scheduled **`ainl_run`**: **`bundle.ainlbundle`** + **`AINL_BUNDLE_PATH`** — **`docs/scheduled-ainl.md`**, **`docs/data-directory.md`**, repo **`ARCHITECTURE.md`**.
