...
**Per-agent graph memory (SQLite):** **`~/.armaraos/agents/<id>/ainl_memory.db`** (**`ainl-memory`** via **`GraphMemoryWriter`** in **`openfang-runtime`**: completed turns, successful tool facts, in-process **`agent_delegate`** + outbound **`a2a_send`**, **persona** → system prompt). Canonical doc: **`docs/graph-memory.md`**. Optional higher-level crate **`ainl-runtime`** (not on the live chat path yet): sync **`run_turn`** / Tokio **`run_turn_async`** with feature **`async`** — **`docs/ainl-runtime.md`**, **`crates/ainl-runtime/README.md`**. Scheduled **`ainl_run`**: **`bundle.ainlbundle`** + **`AINL_BUNDLE_PATH`** — **`docs/scheduled-ainl.md`**, **`docs/data-directory.md`**, repo **`ARCHITECTURE.md`**.
