...
**Support diagnostics + Home folder downloads:** Redacted zip: `POST /api/support/diagnostics`, then `GET /api/support/diagnostics/download?name=<bundle_filename>` (loopback may skip Bearer when `api_key` is set). Full file under home: `GET /api/armaraos-home/download?path=support/…` (256 MiB cap; **read** preview stays 512 KiB). Desktop Tauri: **`copy_diagnostics_to_downloads`** `{ bundlePath }`, **`copy_home_file_to_downloads`** `{ relativePath }` — see **`docs/desktop.md`** (*IPC Commands*). Home folder UI (View vs Download): **`docs/dashboard-home-folder.md`**. Manual QA: **`docs/dashboard-testing.md`** (*Support diagnostics bundle*, *Home folder browser — preview vs download*); smoke: **`./scripts/verify-dashboard-smoke.sh`**.
