...
**Per-agent graph memory (SQLite):** **`~/.armaraos/agents/<id>/ainl_memory.db`** (**`ainl-memory`** via **`GraphMemoryWriter`** in **`openfang-runtime`**: completed turns, successful tool facts, in-process **`agent_delegate`** + outbound **`a2a_send`**, **persona** → system prompt). Canonical doc: **`docs/graph-memory.md`**. Scheduled **`ainl_run`**: **`bundle.ainlbundle`** + **`AINL_BUNDLE_PATH`** — **`docs/scheduled-ainl.md`**, **`docs/data-directory.md`**, repo **`ARCHITECTURE.md`**.
