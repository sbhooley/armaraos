...
| **ainl-runtime** | Standalone **AINL** host runtime crate (depends on **`ainl-memory`**). Optional **`async`** feature: **`run_turn_async`** + **`spawn_blocking`** for SQLite; graph under **`Arc<std::sync::Mutex<_>>`** (not **`tokio::sync::Mutex`**) so **`new` / `sqlite_store`** work on any thread—see **`crates/ainl-runtime/README.md`**. Not required for the default daemon graph. |
