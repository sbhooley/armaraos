...
openfang-memory         SQLite memory substrate, sessions, semantic search, usage tracking
    |
ainl-memory             Standalone graph-memory SQLite (per-agent ainl_memory.db; episodes + persona for prompt)
...
| **openfang-runtime** | Agent execution engine. Contains the agent loop (`run_agent_loop`, `run_agent_loop_streaming`), 3 native LLM drivers (Anthropic, Gemini, OpenAI-compatible covering 20 providers), 23 built-in tools, WASM sandbox (Wasmtime with dual fuel+epoch metering), MCP client/server (JSON-RPC 2.0 over stdio/SSE), A2A protocol (AgentCard, task management), web search engine (4 providers: Tavily/Brave/Perplexity/DuckDuckGo), web fetch with SSRF protection, loop guard (SHA256-based tool loop detection), session repair (history validation), LLM session compactor (block-aware), Merkle hash chain audit trail, embedding driver, and optional **`ainl-memory`** writes via **`GraphMemoryWriter`** (**`~/.armaraos/agents/<id>/ainl_memory.db`**) for delegation episodes, semantic facts, and **persona** nodes (recent traits are appended to the **system prompt** before each LLM call). At loop start, **`GraphMemoryWriter::drain_python_graph_memory_inbox`** merges **Python** **`ainl_graph_memory_inbox.json`** when **`ARMARAOS_AGENT_ID`** is used (**`docs/graph-memory-sync.md`**). Post-turn graph enrichment: optional **`ainl-graph-extractor`** + **`ainl-semantic-tagger`** bridges (env **`AINL_EXTRACTOR_ENABLED`**, **`AINL_TAGGER_ENABLED`**), **`run_persona_evolution_pass`**, and optional **`PersonaEvolutionHook`** (**`AINL_PERSONA_EVOLUTION`**—see **`docs/persona-evolution.md`**). Also ships **`ainl_bundle_cron`** helpers used by the kernel for scheduled **`ainl_run`** bundle env + export. Defines the `KernelHandle` trait that enables inter-agent tools without circular crate dependencies. |
...
| **ainl-runtime** | Standalone **AINL** host runtime crate (depends on **`ainl-memory`**). Turn pipeline (**`run_turn`** / optional **`run_turn_async`**): internal **delegation depth** cap (**`RuntimeConfig::max_delegation_depth`**, hard **`AinlRuntimeError::DelegationDepthExceeded`**). Optional **`async`** feature: **`run_turn_async`** + **`spawn_blocking`** for SQLite; graph under **`Arc<std::sync::Mutex<_>>`** (not **`tokio::sync::Mutex`**) so **`new` / `sqlite_store`** work on any thread—see **[ainl-runtime.md](ainl-runtime.md)** and **`crates/ainl-runtime/README.md`**. Not required for the default daemon graph. |
...
+------------------+
| ainl-memory      |
| GraphMemory      |  ~/.armaraos/agents/<id>/ainl_memory.db; episodes + persona (prompt hook)
| +------------+   |
+------------------+
         |
         v
