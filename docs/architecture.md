...
ainl-runtime            Graph turn orchestration over ainl-memory; optional openfang-runtime feature ainl-runtime-engine
    |
...
- **MCP Server** (`mcp_server.rs`): Exposes ArmaraOS's built-in host tools via the MCP protocol. Enables external tools to use ArmaraOS as a tool provider.
