...
| **Default agent loop graph toggles** | Unrelated to **`ainl-runtime-engine`** — the built-in loop uses **`AINL_EXTRACTOR_ENABLED`** (opt-out; default on), **`AINL_TAGGER_ENABLED`** (opt-out; default on), **`AINL_PERSONA_EVOLUTION`** (opt-out; default on), and export path **`AINL_GRAPH_MEMORY_ARMARAOS_EXPORT`** as documented in **[graph-memory.md](graph-memory.md)** and **`crates/openfang-runtime/README.md`**. |
