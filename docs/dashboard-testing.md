...
**Quick actions, layout, Operations snapshot (integration row + kernel KPIs), Setup Wizard visibility, and CSS:** See **[dashboard-overview-ui.md](dashboard-overview-ui.md)** (section order, `!loadError && !loading` visibility, **seven-tile** Quick actions skeleton + **App Store** + **Daemon & runtime** actions, `openfang-onboarded` / `navigateOverview`, hash targets).
...
3b. **Get started → Token economics + four-tile row** — When a global budget is configured, confirm the **Token Economics** card appears (conditional). Below it, confirm a **single row** of **four** stat cards: **Tokens used**, **Total cost**, **Tokens saved (est.)**, **Cost saved (est.)**. *Saved* figures combine **estimated** compression/cache roll-ups (`GET /api/status` → `eco_compression`, 7d) and **estimated** blocked-turn stats (`quota_enforcement`); they are not duplicate provider meters. See [dashboard-overview-ui.md](dashboard-overview-ui.md#measured-vs-estimated-savings).

3c. **Get started → Operations snapshot** — After load, confirm the **Operations snapshot** card shows **Live** and an **integration** row of five tiles: **Channels**, **Skills**, **MCP servers**, **Tool calls**, **Providers**. **Channels** and **Skills** should navigate to `#channels` and `#skills`. When the observability API returns agent counts, the four **kernel** KPIs and the foot (ready/configured channels, last scheduler tick) should appear **below** the integration row. During **loading**, the skeleton should include a four-cell economics strip + snapshot-shaped block. After at least one v15 compression turn has been recorded (provider-billed input tokens persisted), a second foot block should appear with **Pre-compression input (cum.)**, **Provider-billed input (cum.)** (with USD), and **Saved by compression** (tokens + USD) — sourced from `compression_savings.original_input_tokens_total` / `billed_input_tokens_total` / `billed_input_cost_usd_total` on **`GET /api/usage/summary`**. See [dashboard-overview-ui.md](dashboard-overview-ui.md#operations-snapshot-card).

3d. **Get started → LLM Providers** — **Below** the **panel grid** (System health, Security, … MCP), confirm the **LLM Providers** card lists badges and **above** **Recent activity** (not above the MCP panels).
...
7c. **Settings → Vault** — Open **Settings → Vault**. Confirm the **Feature dependencies** table loads, the catalog lists keys (static + provider `api_key_env` rows) with **Present** / **Source** and a telemetry cell (or **Rotation suggested** when applicable). **Set or rotate:** pick a key, enter a value, **Save to vault** — row should show present after refresh. **Test** semantics: for `GITHUB_TOKEN` / `GH_TOKEN` an actual `GET https://api.github.com/user` runs; the toast shows success and the telemetry cell switches to a fresh **Last set** / test stamp. For LLM provider keys (or other non-testable keys) the request returns `applicable: false` with a friendly message pointing at **Settings → Providers → Test**, **and the row’s telemetry is intentionally not updated** (a non-applicable test never marks the row as “last test failed”). **Remove** should confirm, then clear presence (dev token optional). `GET /api/secrets/catalog` and `GET /api/secrets/dependencies` are public (metadata only); mutations require the usual API key or dashboard session. See [api-reference.md](api-reference.md#vault-and-credentials).
...
| Chat unread (per agent) | **Client state** in **`Alpine.store('app').chatUnreadCounts`**: WebSocket `response` / `canvas` (`armaraos-agent-ws`), digest poll **`GET /api/agents/{id}/session/digest`**, kernel **`Message`→`agent`** (inter-agent), and kernel **`AgentAssistantReply`** (see **`notify_chat_replies`**) all call **`bumpAgentChatUnread`**. **`notifyCenter.syncChatUnreadRows()`** mirrors those counts into panel rows (id **`chat-unread-<agentId>`**), same totals as the **All Agents** nav badge, **Quick open** dots, and **Fleet Status** red badge. **Dismiss** on a chat row clears that agent’s unread. |
| Kernel events (crash, quota, health, cron, workflow finished, …) | Same SSE client as sidebar: **`GET /api/events/stream`** → `notifyCenter.ingestKernelEvent` (assistant-reply *notifications* are unified with the chat-unread table above, not a separate `agent-reply-*` id) |
...
| Blog (ainativelang.com) | Every **1 hour** (and on dashboard load): **`GET https://ainativelang.com/blog/feed.json`** → `syncBlogFeedFromJson`. On **first successful** fetch, posts beyond the newest **three** are recorded as **archive** and never get bell rows (fresh installs do not spam the full archive). Per-post dismiss id **`blog-<slug>`**; persisted slugs **`armaraos-notify-dismissed-blog-slugs`**. Seed / archive list: **`armaraos-blog-notify-seeded-v1`**, **`armaraos-blog-notify-ignored-slugs`**. Row **`Open`** uses dashboard **`armaraosOpenExternalUrl`** for `https://` links (Tauri opens system browser). **`kind`** **`blog`**. |
...
**Persistence:** Dismissing a **kernel** row (id `k-<event id>`) stores that id in **`localStorage`** key **`armaraos-notify-dismissed-kernel`** (capped list) so a full page reload does not show the same SSE event again. Synthetic rows **`approval-pending`** and **`budget-alert`** are not stored there. **GitHub release** rows use **`armaraos-notify-dismissed-release-tag`** (last dismissed tag). **AINL PyPI** rows use **`armaraos-notify-dismissed-ainl-pypi`** (last dismissed PyPI version string). **Blog** rows store dismissed post slugs in **`armaraos-notify-dismissed-blog-slugs`** (capped array); archive seeding uses **`armaraos-blog-notify-seeded-v1`** and **`armaraos-blog-notify-ignored-slugs`**.

**Agent reply noise:** Settings → **System** → **Notification center** — **`notify_chat_replies`** in **`ui-prefs.json`**: **`all`** (kernel **`AgentAssistantReply`** events increment unread and refresh **`chat-unread-*`** panel rows + sidebar), **`hidden`** (no row/unread bump while that agent’s inline chat is the focused, visible **tab**), **`off`** (no kernel-driven unread from assistant replies). Local mirror: **`armaraos-notify-chat-replies`**. WS/digest inter-agent traffic still uses **`bumpAgentChatUnread`** as before.
...
1. **Bell + panel** — Confirm the bell appears when connected. Click it: panel opens; **badge** matches row count. **Esc** and clicking the **dimmed backdrop** close the panel. **Tab** cycles controls inside the panel while open. **Open** on a row navigates via hash and closes the panel. With unread assistant activity, **chat-unread-*** rows and sidebar **All Agents** / **Quick open** / **Fleet Status** badges should show the **same** aggregate logic (dismiss a chat row = clear that agent’s unread).
2. **Approvals** — With at least one **pending** execution approval (`GET /api/approvals`), expect a **Pending approval(s)** row within a few seconds (poll) or immediately after the kernel emits **`ApprovalPending`** on `GET /api/events/stream` (same SSE path as sidebar **SSE**). **Dismiss** hides that row until the set of pending request ids changes (new approval or all resolved). **Clear all** removes every row, resets snooze/debounce state used by the center, and clears in-memory **chat** unread (`chatUnreadCounts` / `chatUnreadPreview`); it does **not** clear kernel-dismissed ids in `localStorage` for one-off **k-*** event rows.
...
4. **Kernel-driven rows** — From live SSE, confirm new rows can appear for **agent crashed** (Lifecycle **Crashed**), **Quota enforced** / **Quota warning**, **Health check failed** (debounced per agent like toasts, ~90s), **Cron job** finished/failed (AINL vs agent-turn titles when **`action_kind`** is set), and **workflow finished**. **New assistant output** is represented by **`chat-unread-` + agent id** rows (unified with sidebar unread), including when **`AgentAssistantReply`** is emitted (subject to **`notify_chat_replies`**). Each **kernel** row (id **k-***) should be individually **Dismiss**-able; after dismiss + reload, the same event id should not reappear. **Chat-unread** rows clear via **Dismiss** (per agent) or **Clear all** (all agents). **Release** / **AINL PyPI** dismissals persist via the keys above.
...
- **Skills → MCP:** Open **#skills** → **MCP Servers** (tab click loads **`loadMcpPanel`**: **`GET /api/mcp/servers`**, **`GET /api/integrations/mcp-presets`**, **`GET /api/integrations/available`**, **`GET /api/integrations`**, and **`GET /api/system/mcp-host-readiness`**). At the top, the **Host tools for google-workspace-mcp** card should explain **uv** / **uvx**, link to **Settings → Tools** (sessionStorage **`armaraos-settings-tab=tools`** + hash **`#settings`**), and when **`uvx`** is missing offer **Run uv installer** / **Copy install** (**`POST /api/system/bootstrap-uv`** on Unix). Confirm the primary **Add custom MCP server** form calls **`POST /api/integrations/custom/validate`** and **`POST /api/integrations/custom/add`**, then **`POST /api/integrations/:id/reconnect`** as needed. Expand **Preset examples** and confirm presets load, **Google Workspace** shows **Client ID** / **Client Secret** fields from the template, and **Validate** / **Install** / **Reconnect** call **`POST /api/integrations/validate`**, **`POST /api/integrations/add`** (with `env`/`config`), and **`POST /api/integrations/:id/reconnect`**. **Settings → Tools** should offer the same OAuth save path (**`POST /api/integrations/google-workspace/oauth`**) and readiness. After install, **`GET /api/mcp/servers`** should list the server under **`configured`** and (when healthy) under **`connected`**.
- **Usage / cost row:** **Tokens used** and **Total cost** on Get started load from **`GET /api/usage/summary`** (SQLite-backed **actual** usage for completed calls). **Input tokens not billed (est.)** and **USD not spent (est.)** prefer **`/api/usage/summary` → `compression_savings`**: cumulative compression + cache counterfactuals in SQLite, with per-event **catalog** input $/1M, plus all-time **`quota_enforcement`**. If the summary omits `compression_savings` (stale client), the UI falls back to **`GET /api/status`** 7d `eco_compression`. See [api-reference.md](api-reference.md#get-apiusagesummary).
- **Analytics parity:** The **Analytics** page summary tab uses the same **`/api/usage/summary`** store (including **`quota_enforcement`** for **Quota blocks** and **Spend avoided (est.)** tiles when present). **By agent** uses **`GET /api/usage`**, which prefers the same persistent metering totals (see **`source`** per row in [api-reference.md](api-reference.md#get-apiusage)).
...
- **Badges** appear on **All Agents** (sidebar nav count, red pill), the **Quick open** list (unread dot + numeric badge when present), the **All Agents (fleet) → Fleet Status** title row (red count badge, same total as nav), the browser **tab title** in the form `(N) ArmaraOS` when the unread total is positive, and **agent picker** cards when the UI detects new **assistant** activity for an agent while that conversation is not the focused, visible inline chat. **Impl:** `app.js` — `chatUnreadCounts`, `bumpAgentChatUnread`, `clearAgentChatUnread`, `notifyCenter.syncChatUnreadRows()`.
- **Notification center (bell):** The same per-agent state drives one **`chat-unread-<agentId>`** row per agent with outstanding unread (with optional text preview from kernel **`AgentAssistantReply`**). Dismissing a row or **Clear all** updates this state so the bell, sidebar, and Fleet badge stay **in sync**.
- **Sources:** (1) WebSocket frames `response` / `canvas` (broadcast as `armaraos-agent-ws`), (2) kernel SSE `Message` events with `role: agent` and an `Agent` target (e.g. inter-agent), (3) kernel **`AgentAssistantReply`** (gated by **`notify_chat_replies`**, same as Settings → System), (4) **digest polling** every ~24s using `GET /api/agents/{id}/session/digest` so unread still updates if the WS is disconnected or another process appended to the session.
- **Baseline:** The client keeps a per-agent `assistant_message_count` baseline from the digest to avoid double-counting when combined with WS/SSE; opening a chat **primes** the baseline and clears that agent’s unread. After WS/SSE unread, **`armaraosScheduleDigestBaselineSync`** refreshes the baseline so the digest poller does not double-count.
...
3. **Default allowlist merge:** When the allowlist is **non-empty**, the kernel merges core file/network/channel tools plus **AINL MCP** helpers (`mcp_ainl_*`, plus **`mcp_resource_read`**) automatically (see [api-reference.md](api-reference.md#get-apiagentsidtools)); you should see those names present after save/reload even if you did not type them manually. **Empty allowlist** still means “profile defaults” (no merge).
