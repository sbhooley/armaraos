...
| `~/.armaraos/hand_state.json` | **Active hand instances** (App Store / curated hands): per-instance `config` maps, agent ids, and `hand_id` for restore on daemon start. Written on activate/deactivate and on successful **`PUT /api/hands/{hand_id}/settings`**. If this file is missing, bundled hands are still available; previously active hands are re-activated from defaults until you open the dashboard and activate them again. See [api-reference.md](api-reference.md#hands-app-store-endpoints). |
...
| `~/.armaraos/secret_center_telemetry.json` | **Settings → Vault** telemetry: per-key `last_set_at`, last **Test** result, and ~**90d** “stale / rotation suggested” flags. Contains **no secret values** — safe to include in support bundles; delete to reset hints. |
