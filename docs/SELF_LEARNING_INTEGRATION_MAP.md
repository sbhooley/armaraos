...
| **Context-aware retrieval** (`AINLContextCompiler`, 500-tok budget, priority 1-3) | `crates/ainl-context-compiler` (Tier 0) + per-turn `compose_telemetry` in `openfang-runtime` | **Core shipped:** `ContextCompiler::compose` + kernel side-channel. **Next:** `summarizer` / `embedder` feature tiers, richer tool segment split (see Phase 6 row). | `ainl-context-compiler` + hosts |
...
- **`scripts/verify-ainl-context-compiler-feature-matrix.sh`** — Phase 6 §16 gate: optional Cargo feature matrix for `ainl-context-compiler` (update the `DEFAULT_FEATS` list in the script if `Cargo.toml` default features change)
