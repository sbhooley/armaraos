...
|---|---|---|---|
| **Typed graph memory** (`ainl_graph_nodes`, `ainl_graph_edges`, FTS5) | `ainl-memory::GraphMemory` (SQLite, WAL, snapshot/import, 5 node kinds: `Episode`, `Semantic`, `Procedural`, `Persona`, `RuntimeState`) | Plugin adds **`Failure`** node + FTS5 over `embedding_text`. ArmaraOS has neither. | `ainl-memory` (extend) |
...
| **Pattern promotion** (min occurrences + fitness ≥ 0.7 → reusable pattern, EMA fitness) | `ainl-graph-extractor` has `recurrence.rs` + `runner.rs` and writes `Procedural` nodes via `record_pattern`. | Promotion **gate** (min uses + fitness threshold) is implicit; no explicit `Suggested Patterns` block injected with fitness. | `ainl-graph-extractor` (extend) |
| **Failure learning** (`failure_resolutions` table, FTS5 lookup, prevention prompt) | **`ainl-memory`** typed **`Failure`** nodes + **`ainl_failures_fts`**, **`search_failures_for_agent`**, OpenFang **`LearningRecorder`** ingest + **`## FailureRecall`** prompt injection + dashboard **`/api/graph-memory/failures/*`** + SSE **`FailureLearned`**. | Optional **`ainl-failure-learning`** crate for portable hosts; no `failure_resolutions` edge table yet. | `ainl-memory` + `openfang-runtime` (ship); `ainl-failure-learning` (defer) |
...
| **Closed-loop validation** (proposal → strict validate → adopt; tracks accept rate) | None. `ainl-runtime` validates AINL via the existing AINL parser, but has no proposal/adoption ledger. | **Third gap.** | `ainl-improvement-proposals` (new) |
| **Adaptive compression** (per-project mode learning, semantic scoring, cache TTL coordination, adaptive eco by content type) | `ainl-compression` has `EfficientMode` (Off/Balanced/Aggressive), `compress()`, `CompressionMetrics` telemetry, `estimate_semantic_preservation_score()` (✅ parity with plugin's `SemanticScorer`). Missing: per-project profile, cache TTL coordination, content-aware adaptive selection. | Partial — extend in place. | `ainl-compression` (extend with `profiles`, `adaptive`, `cache` modules) |
...
**Wiring:** `crates/openfang-runtime/src/agent_loop.rs` already builds prompts; replace the current static `EfficientMode` lookup with a call into `CacheCoordinator::decide(project_id, recommended_mode)`. Per-agent `armaraos-eco-modes-v1` map (see CLAUDE.md *Ultra Cost-Efficient Mode*) becomes the **manual override**, profiles supply the **automatic default**.
...
| **3 — Pattern promotion gate** | Explicit `min_uses + fitness ≥ 0.7` promotion in `ainl-graph-extractor`; "Suggested Patterns" context block | `ainl-graph-extractor`, `ainl-context-compiler` | reuse `ainl-extractor` | unit + recall tests |
| **4 — Closed-loop validation** | `ainl-improvement-proposals` crate; ledger; `ainl-runtime` validator wrapper | `ainl-improvement-proposals`, `ainl-runtime` | `ainl-improvement-proposals` (off by default initially) | accept/reject ratio test; regression test (rejected proposal never adopted) |
| **5 — Adaptive compression** | `profiles.rs`, `adaptive.rs`, `cache.rs` modules; agent loop integration | `ainl-compression`, `openfang-runtime` | reuse existing — gated by env `AINL_ADAPTIVE_COMPRESSION` initially | per-project profile persistence test; cache-window hysteresis test |
| **6 — Context compiler hoist** | Move blocks into `ainl-context-compiler` crate; add new sources; expose token budget knob | `ainl-context-compiler`, `openfang-runtime`, `ainl-runtime` | `ainl-context-compiler` (default-on) | byte-for-byte equivalence test vs current prompts before flipping default |
...
| `mcp_server/extractor.py` | `crates/ainl-graph-extractor/src/{extractor,recurrence,runner}.rs` (existing) | Add explicit promotion gate (`min_uses=3`, `fitness_threshold=0.7`) |
...
| **3** | Promotion gate refuses to fire when `vitals.gate = Block` for the source episodes. Test in `ainl-graph-extractor`. |
