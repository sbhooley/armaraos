...
Templates set a **`profile`** (e.g. `coding`, `full`). With an **empty** per-agent tool allowlist, the runtime expands that profile to a tool set; **Minimal**, **Coding**, **Research**, **Messaging**, **Automation**, and related presets include **`channel_send`** and **`event_publish`** so agents can notify over configured channels without a `Full` / `*` profile. **Coding**, **Research**, **Messaging**, and **Automation** also include **`hermes_a2a_status`**, **`a2a_discover_hermes`**, and **`a2a_send_hermes`** for the operator-local Hermes A2A bridge (`~/.hermes/a2a.json`). If you set a **non-empty allowlist** (Dashboard **Agents → Config**, or **`PUT /api/agents/{id}/tools`**), you must list every tool the agent needs, including those two for outbound channel alerts (and the Hermes trio if you rely on that bridge). See **[Channel Adapters → Agent Routing](channel-adapters.md#agent-routing)** and **[API Reference → Agent tools](api-reference.md#put-apiagentsidtools)**.
