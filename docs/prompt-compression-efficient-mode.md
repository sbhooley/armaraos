...
**Whole-prompt scope (M1 — Phase 6 of `SELF_LEARNING_INTEGRATION_MAP.md`):** The `original_tokens_est` and `compressed_tokens_est` columns on each `eco_compression_events` row now come from `ainl_context_compiler::ContextCompiler::compose` rather than the previous `user_message.len() / 4 + 1` heuristic. The compiler runs in *measurement mode* on every turn — it scores and per-segment-compresses the **assembled** system prompt + history + user message + (eventually) tool definitions / results, but the LLM still receives the host-assembled prompt unchanged. The kernel reads the resulting whole-prompt totals via the `openfang_runtime::compose_telemetry` side-channel (`record_compose_turn` from the agent loop, `take_compose_turn` from the kernel's compression-event recorder) and falls back to the legacy user-message-only math if the side-channel snapshot is missing. This is what lifts the dashboard's "TOKENS USED" / "USD NOT SPENT" tiles out of the near-zero range — under the previous narrow scope, compression telemetry only saw ~0.006 % of actual billed input tokens. M2 swaps the LLM-bound prompt for `ComposedPrompt.segments`, capturing the savings at the wire as well.

**Free tier / $0 catalog rows:** `input_price_per_million_usd` and counterfactual `est_input_cost_saved_usd` for compression use the model catalog (and `0.0` when a model is unknown) so free routes (e.g. OpenRouter `:free`, built-in $0 entries) do **not** get a legacy `$1/M` default that would inflate "USD not spent" after M1. `openfang_memory::record_compression` stores the kernel’s price snapshot as-is and does not overlay a positive heuristic on top of an explicit `0.0`. Provider-billed `input_tokens` in `usage_events` are unchanged.
