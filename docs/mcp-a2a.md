...
In addition to namespaced `mcp_{server}_*` tools from `tools/list`, ArmaraOS exposes a **built-in** tool **`mcp_resource_read`** that reads a resource **URI** from a **connected** MCP client connection using the server’s JSON-RPC **`resources/read`** (implemented on `McpConnection` in `openfang-runtime::mcp`). Agents use it to load integration text advertised under **`mcp_resources`** on **`mcp_ainl_ainl_capabilities`** — for strict authoring prefer **`ainl://strict-authoring-cheatsheet`**, **`ainl://strict-valid-examples`**, **`ainl://strict-valid-families`** (mined strict-valid **family** index for wizard/corpus workflows), and **`ainl://adapter-contracts`**; **`ainl://authoring-cheatsheet`** and **`ainl://integrations-*`** cover HTTP `R`‑line and integration notes — without paste‑outsourcing.
...
**AINL MCP digest + `recommended_next_tools` in graph memory:** On successful **`mcp_ainl_*`** calls, the loop may persist **semantic** facts (visible under graph memory / FTS) when the payload **hash** changes: tags **`mcp:ainl:capabilities`**, **`mcp:ainl:recommended_next`**, and when the authoring wizard state updates, **`mcp:ainl:wizard_state`**, each with a companion **`v:<sha16>`** tag for dedupe. The latest wizard digest can be re-read on later turns via **`read_wizard_state_from_graph`** (`mcp_ainl_session.rs`) and folded into a short system-prompt line through **`PromptContext.mcp_ainl_wizard_state_hint`** (`prompt_builder.rs` / kernel wiring). Prompt assembly prefers the **latest** of the capabilities / recommended rows for the agent (then falls back to the in-process **`mcp_ainl_session`** cache). **`recommended_next_tools`** is only re-injected into the prompt when its content hash changes (same for the capabilities digest), to avoid steady token tax. Session-warmed digests may include a short **`strict_summary`** plus per-adapter **`[strict_contract]`** markers when the AINL MCP server exposes them. On **`ainl_validate` / `ainl_compile`**, the Python server adds **`contract_validation_status`** and **`contract_alignment.mismatched_calls`**; the ArmaraOS session layer formats a compact **contract alignment** note for the prompt (preferring **`mismatched_calls`**, with a legacy fallback if an older host emits **`items`**). **Failures** from **`mcp_ainl_*`** tools are recorded on **`Failure`** nodes with optional **`source_namespace`** / **`source_tool`** fields (no `[MCP-AINL:…]` message prefix). **In-repo map:** [AI Native Lang `docs/operations/MCP_AINL_WIZARD_AND_CORPUS.md`](https://github.com/sbhooley/ainativelang/blob/main/docs/operations/MCP_AINL_WIZARD_AND_CORPUS.md).
