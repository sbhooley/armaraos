...
**AINL graph memory (process environment):** **`AINL_EXTRACTOR_ENABLED`**, **`AINL_TAGGER_ENABLED`**, **`AINL_PERSONA_EVOLUTION`**, and **`AINL_IMPROVEMENT_PROPOSALS_ENABLED`** are **not** `config.toml` keys—they apply to the **`openfang-runtime`** / daemon process (set in `~/.armaraos/.env` or the service plist, same as other env vars). `AINL_EXTRACTOR_ENABLED` and `AINL_PERSONA_EVOLUTION` are **opt-out** (default on when their Cargo features are compiled in; set to `0`/`false`/`no`/`off` to disable). `AINL_TAGGER_ENABLED` is **opt-in** (must be exactly `1`). **`AINL_IMPROVEMENT_PROPOSALS_ENABLED`** is **opt-out** for the improvement-proposal **HTTP** routes and SQLite ledger (default on when unset; set to `0`/`false`/`no`/`off` to disable). See **[graph-memory.md](graph-memory.md)** (*Extraction, tagging, and persona evolution* env table) and **[SELF_LEARNING_INTEGRATION_MAP.md](SELF_LEARNING_INTEGRATION_MAP.md) §15.7**.
