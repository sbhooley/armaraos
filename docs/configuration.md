...
- To persist env values without hand-editing files, use the embedded dashboard **Settings → Vault** (or **`~/.armaraos/secrets.env`** / **`~/.armaraos/.env`** on disk — see [data-directory.md](data-directory.md)). [api-reference.md](api-reference.md#vault-and-credentials) documents the `GET/POST/DELETE` `/api/secrets` endpoints.
...
**AINL MCP:** `ainl install-mcp --host armaraos` registers an `ainl` server whose `env` entry forwards `AINL_MCP_EXPOSURE_PROFILE`, `AINL_MCP_TOOLS`, `AINL_MCP_TOOLS_EXCLUDE`, `AINL_MCP_RESOURCES`, and `AINL_MCP_RESOURCES_EXCLUDE` from the parent process. Set values in `~/.armaraos/.env` (for example `AINL_MCP_EXPOSURE_PROFILE=inspect_only` for read-first tooling and the `ainl://authoring-cheatsheet` resource) without editing this file; leave `AINL_MCP_EXPOSURE_PROFILE` unset for the full MCP tool list. To **read** a listed resource URI in chat, the embedded agent uses the host built-in **`mcp_resource_read`** tool (MCP client **`resources/read`** on the `ainl` connection), not a separate `pip` install. That tool defaults to **64 KiB** UTF-8 **bytes** of resource text per call (`max_bytes`, optional `char_offset` / `offset` as Unicode-scalar skip, and `allow_binary` for non-text); see **`docs/mcp-a2a.md`** (*Host `mcp_resource_read`*).

**Google Workspace MCP (bundled template `google-workspace-mcp`):** The extension launches **`uvx workspace-mcp`** (PyPI **`workspace-mcp`**). Set **`GOOGLE_OAUTH_CLIENT_ID`** and, for most Desktop OAuth clients, **`GOOGLE_OAUTH_CLIENT_SECRET`** in **`~/.armaraos/.env`**, via the dashboard (**Settings → Tools** or **Skills → MCP** → preset form), or **`POST /api/integrations/google-workspace/oauth`**. Process-only overrides: **`ARMARAOS_AUTO_INSTALL_UV`** — when truthy, the daemon runs the official **uv** installer once at startup (Unix) if **`uvx`** is not available before MCP connects; **`ARMARAOS_WORKSPACE_MCP_COMMAND`** — full custom command line instead of the default **`uvx`** args; **`ARMARAOS_DISABLE_DEFAULT_GOOGLE_WORKSPACE_MCP=1`** — do not auto-inject the default server when a client id is present. See [api-reference.md](api-reference.md#google-workspace-mcp-host-tooling).

**Agent `capabilities.tools` and MCP (manifest, e.g. `~/.armaraos/agents/<name>/agent.toml`):** When the tool list is a **non-empty** allowlist (and not a single `*`), the kernel also merges the glob **`mcp_*`**, so any tool from a **connected** MCP server (names of the form `mcp_<server>_<tool>`) is eligible. Installing or connecting an integration is the trust boundary. You may list **`"mcp_*"`** in the manifest (bundled `agents/*` templates do) or rely on the automatic merge. To **restrict** which servers an agent may use, set a non-empty per-agent **`mcp_servers`** allowlist (in the agent manifest) or do not connect unwanted integrations — empty **`mcp_servers`** still means *all* connected servers.
...
**AINL graph memory (process environment):** **`AINL_EXTRACTOR_ENABLED`**, **`AINL_TAGGER_ENABLED`**, **`AINL_PERSONA_EVOLUTION`**, and **`AINL_IMPROVEMENT_PROPOSALS_ENABLED`** are **not** `config.toml` keys—they apply to the **`openfang-runtime`** / daemon process (set in `~/.armaraos/.env` or the service plist, same as other env vars). `AINL_EXTRACTOR_ENABLED`, `AINL_TAGGER_ENABLED`, and `AINL_PERSONA_EVOLUTION` are **opt-out** (default on when their Cargo features are compiled in; set to `0`/`false`/`no`/`off` to disable). **`AINL_IMPROVEMENT_PROPOSALS_ENABLED`** is **opt-out** for the improvement-proposal **HTTP** routes and SQLite ledger (default on when unset; set to `0`/`false`/`no`/`off` to disable). See **[graph-memory.md](graph-memory.md)** (*Extraction, tagging, and persona evolution* env table) and **[SELF_LEARNING_INTEGRATION_MAP.md](SELF_LEARNING_INTEGRATION_MAP.md) §15.7**.
...
**Graph memory (SQLite + optional export / tagging):** toggles such as **`AINL_EXTRACTOR_ENABLED`**, **`AINL_TAGGER_ENABLED`**, **`AINL_PERSONA_EVOLUTION`**, **`AINL_IMPROVEMENT_PROPOSALS_ENABLED`**, and **`AINL_GRAPH_MEMORY_ARMARAOS_EXPORT`** are **not** `config.toml` keys—they are **process environment** variables consumed by **`openfang-runtime`** when recording turns (and for **`AINL_IMPROVEMENT_PROPOSALS_ENABLED`**, the graph-memory **HTTP** routes / proposal ledger; **opt-out**, default on). `AINL_EXTRACTOR_ENABLED`, `AINL_TAGGER_ENABLED`, and `AINL_PERSONA_EVOLUTION` are **opt-out** (default on; set to `0`/`false`/`no`/`off` to disable). Full semantics in **[`graph-memory.md`](graph-memory.md)** and **[`persona-evolution.md`](persona-evolution.md)**; see also **`.env.example`**.
