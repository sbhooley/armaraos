...
Implementation pointers: **`cron_success_suppresses_session_append`** in **`crates/openfang-kernel/src/kernel.rs`**, **`armaraosRoutineMonitorCronJobName()`** in **`crates/openfang-api/static/js/app.js`**, and **`routine_monitor_cron_quiet_success`** in **`crates/openfang-desktop/src/lib.rs`** (keep all three aligned: curated job names, path markers, and the **`test-ainl-`** name prefix).
