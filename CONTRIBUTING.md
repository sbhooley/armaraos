...
| `ainl-runtime` | Standalone AINL host runtime (depends on `ainl-memory`); workspace packaging / future host wiring. Internal **delegation depth** on nested **`run_turn`** / **`run_turn_async`** (**`DelegationDepthExceeded`**). Optional **`async`** feature: **`run_turn_async`** + Tokio **`spawn_blocking`** for graph SQLite. Verify: `cargo test -p ainl-runtime`, `cargo test -p ainl-runtime --test test_delegation_depth`, and `cargo test -p ainl-runtime --features async` (see **`docs/ainl-runtime.md`**) |
