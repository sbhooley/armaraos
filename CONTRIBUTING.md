...
| `ainl-runtime` | Rust graph-orchestration over **`ainl-memory`** (**`AinlRuntime`**, **`run_turn`** / **`run_turn_async`**). Workspace consumer: **`openfang-runtime`** via default feature **`ainl-runtime-engine`** (enabled in production builds). Not MCP/CLI/IR execution. Internal **delegation depth** (**`DelegationDepthExceeded`**). Optional **`async`** feature. Verify: `cargo test -p ainl-runtime`, `cargo test -p ainl-runtime --test test_delegation_depth`, `cargo test -p ainl-runtime --features async`; bridge check: `cargo test -p openfang-runtime test_agent_loop_uses_openfang_by_default` — **`docs/ainl-runtime.md`** |
| `openfang-runtime` | Agent loop, 3 LLM drivers (Anthropic/Gemini/OpenAI-compat), 38 built-in tools, WASM sandbox, MCP client/server, A2A protocol; default-on **`ainl-runtime-engine`** embeds **`ainl-runtime`** for the graph-first turn path (per-agent switch can still disable) |
