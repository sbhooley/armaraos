...
| `ainl-runtime` | Rust graph-orchestration over **`ainl-memory`** (**`AinlRuntime`**, **`run_turn`** / **`run_turn_async`**). Workspace consumer: **`openfang-runtime`** only, optional feature **`ainl-runtime-engine`** (stock kernel builds leave it off). Not MCP/CLI/IR execution. Internal **delegation depth** (**`DelegationDepthExceeded`**). Optional **`async`** feature. Verify: `cargo test -p ainl-runtime`, `cargo test -p ainl-runtime --test test_delegation_depth`, `cargo test -p ainl-runtime --features async`; optional bridge: `cargo test -p openfang-runtime --features ainl-runtime-engine test_agent_loop_uses_openfang_by_default` — **`docs/ainl-runtime.md`** |
| `openfang-runtime` | Agent loop, 3 LLM drivers (Anthropic/Gemini/OpenAI-compat), 38 built-in tools, WASM sandbox, MCP client/server, A2A protocol; optional **`ainl-runtime-engine`** embeds **`ainl-runtime`** for an alternate graph-only turn path |
