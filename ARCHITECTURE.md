...
- **openfang-runtime**: Agent execution engine, tool runner, loop guard; default-built Cargo feature **`ainl-runtime-engine`** can route a turn through **`ainl-runtime`** (`AinlRuntime::run_turn`) instead of the LLM loop when enabled per manifest / env — see **`docs/ainl-runtime-integration.md`**. Nested **`run_turn`** depth is enforced **inside** **`ainl-runtime`** (**`AinlRuntimeError::DelegationDepthExceeded`**; **`TurnInput::depth`** is metadata only — **`docs/ainl-runtime.md`**).
