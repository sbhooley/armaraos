...
- **`docs/ainl-runtime.md`** — hub: intro paragraph (**`TurnInput::depth`** vs internal cap); *Where to read next*; **`RuntimeStateNode`** + **`ExtractionReport`**, **`run_turn` / `run_turn_async`**, **`async`**, Mutex vs Tokio, hooks; delegation / **`AinlRuntimeError`** / **`test_delegation_depth`**; verification (**`test_session_persistence`**) + **`required-features`**, embedding caveats.
...
- **`docs/README.md`** — graph-memory blurb (**per-phase** **`ExtractionReport`** / **`warn!`**); Integrations hub row (**`TurnOutcome`** / **`TurnPhase`** / **ExtractionReport** slot names); Reference table tweaks.
...
- **`CLAUDE.md`** — graph-memory overview: **`ExtractionReport`** per-slot **`warn!`** vs **`ainl-runtime`** **`TurnPhase`** **`TurnWarning`**s.
