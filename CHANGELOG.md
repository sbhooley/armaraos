...
- **Curated cron + Chat:** **[docs/scheduled-ainl.md](docs/scheduled-ainl.md)** (*Session transcript, notifications, and routine monitors*), **[docs/ootb-ainl.md](docs/ootb-ainl.md)** (health/budget rows + “where output goes”), **[docs/dashboard-design-system.md](docs/dashboard-design-system.md)** (*Chat composer*), and **[docs/README.md](docs/README.md)** (Scheduled AINL + OOTB index rows).
...
- **Dashboard + kernel — curated cron “routine” monitors:** Successful runs of the embedded **health / budget / weekly AINL-smoke** jobs no longer append scheduler output to the **agent Chat** transcript, and the dashboard does not show **success** toasts or notification-center rows for those jobs (failures still alert). Aligned: **`cron_success_suppresses_session_append`** in **`openfang-kernel`** and **`armaraosRoutineMonitorCronJobName()`** in **`app.js`**.
