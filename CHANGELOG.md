...
- **AINL MCP — failed `ainl_validate` / `ainl_compile` / `ainl_run` now forces repair before finalization:** Semantic authoring failures now create a pending repair obligation in the agent loop. If the next model turn tries to produce a final or silent response without first editing/retrying the failed AINL tool, the loop injects a system prompt that forbids success claims, forbids `strict=false` downgrades, and requires the same tool to return `{"ok": true, ...}`. Malformed JSON or missing `ok` on AINL authoring tools is also treated as unsafe. Near-limit wrap-up and max-step exits now preserve the failure outcome instead of converting it to success; the kernel records these safe returns as turn failures. Tests: `agent_loop::tests::ainl_validate_*`, `agent_loop::tests::ainl_compile_and_run_*`, `mcp_ainl_session::tests::soft_failure_*`.
- **Agent loop hardening — safer tool rounds and outcome-aware learning:** Tool rounds now classify execution lanes so adjacent read-only calls can still parallelize while workspace mutations, shell/process tools, and external side effects run serially in LLM-declared order. Loop-guard outcome tracking is wired after execution, duplicate tool-call collapse is limited to safe read-only calls, graph-memory vitals selection is shared between streaming and non-streaming paths, trajectory rows use non-success outcomes for safe failures, and post-turn persona/background learning has observable `post_turn_learning_metrics` in `GET /api/status`.

### Added

- **Agent loop — AINL MCP adapter-registration repair path (C2/C4):** When `mcp_ainl_*` tool errors look like MCP `adapter_registration` / `suggested_adapters` failures, the loop now (1) adds a repair supplement that tells the model to fix `adapters` configuration instead of deleting `fs`/`http` lines, (2) prioritizes a stronger `corrective_hint_for_failures` for that class, and (3) on the **second** identical tool-error streak, escalates with an AINL-specific message (copy `suggested_adapters`, do not pivot to an unrelated method) instead of the generic “switch to a completely different method” text. Tests: `agent_loop::tests::{looks_like_ainl_adapter_registration_failure_detects_mcp_envelope, ainl_mcp_batch_is_adapter_registration_loop_requires_all_mcp_ainl, tool_error_tracker_second_streak_adapter_registration_escalation, ainl_repair_prompt_includes_adapter_registration_supplement_for_run}`. File: `crates/openfang-runtime/src/agent_loop.rs`.

- **AINL MCP prompt/digest — strict hints in session capabilities snapshot:** The AINL capabilities digest injected into prompts can now include `strict_summary` and per-adapter `[strict_contract]` markers when present in `mcp_ainl_ainl_capabilities`, and the AINL authoring workflow guidance now points agents at `mcp_ainl_ainl_get_started` plus strict MCP resources (`ainl://strict-authoring-cheatsheet`, `ainl://strict-valid-examples`, `ainl://adapter-contracts`). Files: `crates/openfang-runtime/src/mcp_ainl_session.rs`, `crates/openfang-runtime/src/prompt_builder.rs`.
- **AINL authoring hint (context compiler) — richer `mcp_ainl_ainl_run` adapters cheatsheet:** The compressed `mcp_ainl_ainl_run` adapters cheatsheet now reminds that validate/compile `ok: true` is compiler-only and highlights strict resources and the get-started wizard for unfamiliar graphs. File: `crates/ainl-context-compiler/src/mcp_ainl_prompt.rs`.
...
- **Self-learning — reusable procedure artifacts and `procedure_mint` proposals:** Added portable `ExperienceBundle`, `ProcedureArtifact`, `ProcedurePatch`, lifecycle, reuse-outcome, and render-target contracts to `ainl-contracts`; new standalone `ainl-procedure-learning` crate distills recurrent experience into deterministic procedure candidates, scores reuse, renders `SKILL.md` / `skill.toml` / AINL compact skeleton / Hand metadata, and applies failure-aware patches without any `openfang-*` dependency. `ainl-trajectory` now clusters trajectories into `ExperienceBundle`s, `ainl-failure-learning` emits structured `ProcedurePatch` candidates, `ainl-improvement-proposals` recognizes `procedure_mint` / `procedure_patch` / promote / deprecate kinds, and `ainl-memory` can persist validated `ProcedureArtifact` JSON in procedural graph nodes. On recurrence, OpenFang now stages a companion `procedure_mint` proposal next to `pattern_promote` (env **`AINL_AUTO_SUBMIT_PROCEDURE_PROPOSALS`**, default **on**; telemetry in `post_turn_learning_metrics`). The existing Graph Memory proposal API can review/list/validate/adopt procedure proposals; `procedure_mint` adoption materializes a prompt-eligible procedural graph node, and `procedure_patch` proposals validate as JSON before operator adoption. This completes the reusable core of the “experience → procedure artifact → reuse → patch on failure” loop.
...
- **MCP `mcp_resource_read` — strict authoring URIs and digest markers:** Updated the MCP/A2A guide to prefer strict authoring resources and note that session-warmed AINL digests may include `strict_summary` and `[strict_contract]` markers. Doc: `docs/mcp-a2a.md`.
